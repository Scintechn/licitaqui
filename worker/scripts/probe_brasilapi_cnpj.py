...
def fetch(cnpj: str) -> dict:
    ...
    try:
        with urllib.request.urlopen(req, timeout=TIMEOUT, context=SSL_CTX) as resp:
            ...
            return {"cnpj": cnpj, "ok": True, "status": resp.status,
        ...
        try:
            ...
            msg = ""
        return {"cnpj": cnpj, "ok": False, "status": exc.code, "s": time.monotonic() - started,
                "err": f"http_{exc.code}", "msg": msg,
                "retry_after": exc.headers.get("retry-after"),
                "ratelimit": {k: v for k, v in exc.headers.items()
        ...
        return {"cnpj": cnpj, "ok": False, "status": 0, "s": elapsed, "err": err,
                "msg": str(exc)[:140]}
...
def main() -> int:
    ...
    parser.add_argument("--burst", type=int, default=12,
                        help="back-to-back lookups used to probe the rate limit")
    ...
    for i, cnpj in enumerate(cnpjs, 1):
        row = fetch(cnpj)
        rows.append(row)
        print(f"  {i:>3}/{len(cnpjs)} {cnpj} {row['status']:>4} {row['s']:5.2f}s "
              f"{row.get('err', '')} {row.get('msg', '')[:70]}", flush=True)
    ...
    for cnpj in cnpjs[: args.burst]:
        row = fetch(cnpj)
        burst.append(row)
        print(f"  burst {cnpj} {row['status']:>4} {row['s']:5.2f}s {row.get('err', '')} "
              f"{row.get('ratelimit', '')}", flush=True)
    ...
    report = {
        "sequential": {
            "n": len(rows), "ok": len(ok), "failed": len(rows) - len(ok),
            "failure_rate_pct": (round(100 * (len(rows) - len(ok)) / len(rows), 1)
                                 if rows else 0.0),
            "errors": errors, "delay_s": args.delay,
            "latency_ok_s": {
                "min": round(lat[0], 2),
        ...
        "burst": {"n": len(burst), "statuses": statuses,
