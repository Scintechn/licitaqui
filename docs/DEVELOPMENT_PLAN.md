...
| G2 | Email: Resend **test sender** during development (delivers only to the account owner, enough to test every flow). A verified sending domain is needed before emailing real users | account | email to real users (magic link, welcome, opening) | ✅ test sender now · domain later. Until then real users get Google login and WhatsApp messages | before 10-08 if email login should be public |
...
| G7 | Spike: PNCP period endpoints (`/v1/contratacoes/publicacao`, `/atualizacao`) vs. the search API used in the POCs | technical | M2 sync design | Default: keep the POC search API | **09-25** |
| G8 | Spike: BrasilAPI CNPJ (fields, rate limits, failure rate) | technical | M2 CNPJ entry | Default: manual CNAE fallback | **09-25** |
...
| **M3 Founders opening** | Thu 10-08, 19:00 | Opening notice by WhatsApp (Evolution API); accounts (Google, plus magic link when the sending domain is verified), visitor 3-day/2-screening limits, files unlocked with account, Telegram linking, weekly digest job, `/admin` gate dashboard | Founders receive access email; Telegram `/start` links in one tap; quotas enforced server-side |
...
| **M5 Billing live** | Thu 10-29 (sandbox done 10-23) | Asaas subscriptions (Promocional R$ 26, Essencial R$ 57) on sandbox then production, checkout link, idempotent webhook, `promo_ends_on` + 30-day notice + automatic change to R$ 57, 1-click cancel | Full sandbox cycle incl. repeated webhooks and price change simulated with a clock override; subscription link sent to the 48 founders |
...
| **v1 Essencial** | build 11-09 → 01-08 · release **Mon 2027-01-11** | Winning price band by state, target purchase price + margin calculator, daily alerts (10 keywords + CNAE), ME/EPP and value filters, deep analysis with quota, billing reminders | Gate 1: ≥ 15 paying Essencial-equivalents, month-1 churn < 10% |
