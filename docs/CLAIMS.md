...
| Editais named as closing **30/09** in example panels | `radar.landing.alerts.items[0].note` (*"encerra 30/09"*); also `radar.landing.opportunity.deadlineValue` and `foundersPage.screening.buyer` | **Verified 2026-09-25 — this row overstated it, and two of the three are already dated.** `opportunity` carries `source` = *"…como estava em {data}"*, bound to `EXAMPLE_AS_OF` at `sections.tsx:172`; the `/fundadores` screening panel carries `cardLabel` *"Exemplo de triagem por IA"* and `published` *"Edital publicado no PNCP em 16/09/2026"*. A past date under an as-of line is stale, not false — which is the standard D11 itself settled on. **The one real gap is the alerts panel**: `when` is *"segunda, 07:00 · exemplo"* and nothing in it names a date, so *"encerra 30/09"* reads as a live claim forever. Needs a sentence from Sci, not code. Separately, and smaller: the `opportunity` figure keeps *"Proposta até 30/09"* after the date, where `ExampleRadar` degrades to the approved *"Data anterior"* (`radar.card.previousDeadline`) — code, but the label case is Sci's pick | **D11b** | **30/09** |
