...
| 2026-09-18 | B6 (CNAE → segment map, gap G6) | in review | https://github.com/Scintechn/licitaqui/pull/16 | 572 mappings over 555 of 1332 CNAE subclasses; 777 deliberately unmapped. 20 real CNPJs: **19/20**, the one miss was a wrong expectation, not a wrong mapping. **Rule changed mid-task on measurement:** `compatible` requires the main CNAE — strongest-claim-wins made the average company compatible with 5.75 of 14 segments (one with 13), against 0.8 now. Needs Sci's 150-code review, starting with the goods-vs-services line (`db/reference/README.md`). For B3: POC 1 leaves NCM ch.22 (beverages) in "Outros", so no company ever matches a water tender |
