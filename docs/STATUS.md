...
| 2026-09-18 | B3 (`sync_items` + segments) | in review | TBD | Classification matches POC 1 on 489 real cached items (labels generated by the POC, re-derived from it locally), with **one deliberate divergence**: beverages (NCM chapter 22) are rescued from "Outros" — B6's finding, 8 of 8,861 items move, all beverages. Segment vocabulary is **English keys** with pt-BR labels — B6 and R1 must use the same keys. Neither PNCP bug applies to items: item `orcamentoSigiloso` is a real boolean, and no `tender_items` timestamptz is fed by PNCP. Naive-time and value-zeroing details in `worker/README.md` |
